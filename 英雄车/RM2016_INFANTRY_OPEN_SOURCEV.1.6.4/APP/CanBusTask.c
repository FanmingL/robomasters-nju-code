...
void Set_CM_Speed2(CAN_TypeDef *CANx, int16_t cm1_iq)
{
    CanTxMsg tx_message;
    tx_message.StdId = 0x204;
    tx_message.IDE = CAN_Id_Standard;
    tx_message.RTR = CAN_RTR_Data;
    tx_message.DLC = 0x08;
		tx_message.Data[0] = (uint8_t)(cm1_iq >> 8);
    tx_message.Data[1] = (uint8_t)cm1_iq;
    tx_message.Data[2] = (uint8_t)(0 >> 8);
    tx_message.Data[3] = (uint8_t)0;
    tx_message.Data[4] = (uint8_t)(0 >> 8);
    tx_message.Data[5] = (uint8_t)0;
    tx_message.Data[6] = (uint8_t)(0 >> 8);
    tx_message.Data[7] = (uint8_t)0;
    CAN_Transmit(CANx,&tx_message);
}
