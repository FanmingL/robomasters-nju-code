	...
	switch(friction_wheel_state)
	{
		...
		case FRICTION_WHEEL_START_TURNNING:
		{
			if(sw->switch_value1 == REMOTE_SWITCH_CHANGE_3TO1)   //刚启动就被关闭
			{
				LASER_OFF();
				SetShootState(NOSHOOTING);
				SetFrictionWheelSpeed(1000,1000);
			...
			else
			{
				...
				1000 + (FRICTION_WHEEL_MAX_DUTY_BIG-1000)*frictionRamp.Calc(&frictionRamp)); 
...
void RemoteDataPrcess(uint8_t *pData)
{
    if(pData == NULL)
    {
       ...
       RC_CtrlData.rc.ch1 = ((int16_t)pData[0] | ((int16_t)pData[1] << 8)) & 0x07FF; 
    RC_CtrlData.rc.ch0 = (((int16_t)pData[1] >> 3) | ((int16_t)pData[2] << 5)) & 0x07FF;
...
//	SetFrictionWheelSpeed(1000+(RC_CtrlData.rc.ch3 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_PITCH_ANGLE_INC_FACT);
	switch(GetInputMode())
	{
		case REMOTE_INPUT:
		{
...
void RemoteControlProcess(Remote *rc)
{
 ...
 //LFM       GimbalRef.yaw_angle_dynamic_ref   += (rc->ch2 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_YAW_ANGLE_INC_FACT;      	
			GimbalRef.yaw_angle_dynamic_ref   = (rc->ch2 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_YAW_ANGLE_INC_FACT;  //LFM
	}
...
{
	VAL_LIMIT(GimbalRef.pitch_angle_dynamic_ref, -PITCH_MAX+6, PITCH_MAX-6);
//LFM	VAL_LIMIT(GimbalRef.yaw_angle_dynamic_ref, GMYPositionPID.fdb - 60, GMYPositionPID.fdb + 60);
}
