...
void PrintFSM(void)
{
	switch(workState)
	{
		case PREPARE_STATE:printf("PREPARE_STATE\n");break;
		case STANDBY_STATE:printf("STANDBY_STATE\n");break;
		case NORMAL_STATE:printf("NORMAL_STATE\n");break;
		case STOP_STATE:printf("STOP_STATE\n");break;
		case CALI_STATE:printf("CALI_STATE\n");break;
 ...
 void PrintError(void)
 {
	...
	{
		printf("LOST_ERROR_NOCALI\n");
	}	
	if(Get_Lost_Error(LOST_ERROR_IMU) == LOST_ERROR_IMU)
	{
		printf("LOST_ERROR_IMU\n");
	}
	if(Get_Lost_Error(LOST_ERROR_ZGYRO) == LOST_ERROR_ZGYRO)
	{
		printf("LOST_EEROR_ZGYRO\n");
	}
	if(Get_Lost_Error(LOST_ERROR_MOTOR6) == LOST_ERROR_MOTOR6)
	{
		printf("LOST_ERROR_MOTOR6\n");
	}	
	if(Get_Lost_Error(LOST_ERROR_MOTOR5) == LOST_ERROR_MOTOR5)
	{
		printf("LOST_ERROR_MOTOR5\n");
	}	
	if(Get_Lost_Error(LOST_ERROR_MOTOR1) == LOST_ERROR_MOTOR1)
	{
		printf("LOST_ERROR_MOTOR1\n");
	}
	if(Get_Lost_Error(LOST_ERROR_MOTOR2) == LOST_ERROR_MOTOR2)
	{
		printf("LOST_ERROR_MOTOR2\n");
	}
	if(Get_Lost_Error(LOST_ERROR_MOTOR3) == LOST_ERROR_MOTOR3)
	{
		printf("LOST_ERROR_MOTOR3\n");
	}
	if(Get_Lost_Error(LOST_ERROR_MOTOR4) == LOST_ERROR_MOTOR4)
	{
		printf("LOST_ERROR_MOTOR4\n");
	}
	if(Get_Lost_Error(LOST_ERROR_RC) == LOST_ERROR_RC)
	{
		printf("LOST_ERROR_RC\n");
	}
	if(Get_Lost_Error(LOST_ERROR_DEADLOCK) == LOST_ERROR_DEADLOCK)
	{
		printf("LOST_ERROR_DEADLOCK\n");
	}
 }
...
{  
	...
	//	printf("PREPARE_STATE\n");
	}
	else
	{
		 //底盘跟随编码器旋转PID计算
		//LFM CMRotatePID.ref = 0;
		//LFM CMRotatePID.fdb = GMYawEncoder.ecd_angle;
		CMRotatePID.ref = -GimbalRef.yaw_angle_dynamic_ref;
		CMRotatePID.fdb = MPU6050_Real_Data.Gyro_Z;
	//	printf("ref:%.2f,fdb:%f\n",CMRotatePID.ref,CMRotatePID.fdb);
	...
	//printf("for_back_ref   %d,left_right_ref%d\n",ChassisSpeedRef.forward_back_ref,ChassisSpeedRef.left_right_ref);
	//	printf("Ref1:%f,Ref2:%f,Ref3:%f,Ref4:%f\n",CM1SpeedPID.ref,CM2SpeedPID.ref,CM3SpeedPID.ref,CM4SpeedPID.ref);
...
//	printf("Ref1:%d,Ref2:%d,Ref3:%d,Ref4:%d\n",CM1Encoder.filter_rate,CM2Encoder.filter_rate,CM3Encoder.filter_rate,CM4Encoder.filter_rate);
	...
	//	 Set_CM_Speed(CAN1, (int16_t)(CHASSIS_SPEED_ATTENUATION * CM1SpeedPID.output), (int16_t)(CHASSIS_SPEED_ATTENUATION * CM2SpeedPID.output), (int16_t)(CHASSIS_SPEED_ATTENUATION * CM3SpeedPID.output), (int16_t)(CHASSIS_SPEED_ATTENUATION * CM4SpeedPID.output));		 
//		printf("CAN\t%d\t%d\t%d\t%d\n",0,0,0,0);
	 }
	 else
	 {
	///	printf("CAN\t%d\t%d\t%d\t%d\n",(int16_t)(CHASSIS_SPEED_ATTENUATION * CM1SpeedPID.output),(int16_t)(CHASSIS_SPEED_ATTENUATION * CM2SpeedPID.output),(int16_t)(CHASSIS_SPEED_ATTENUATION * CM3SpeedPID.output),(int16_t)(CHASSIS_SPEED_ATTENUATION * CM4SpeedPID.output));
...
//发射机构射击电机任务
int16_t pwm_ccr = 0;
#define BCOUNT_DELAY 300/4/2				//300ms
void ShooterMControlLoop(void)	
{				  
	static int 	BCount=0;
	static int BCSTATE = 1;   //0 Open 1 Close;
	...
	//LFM PWM3 = ShootMotorSpeedPID.output;	
}
...
void Control_Task(void)
{
	time_tick_1ms++;
	WorkStateFSM();										//切换工作模式
	WorkStateSwitchProcess();					//如果从其他模式切换到prapare模式，要将一系列参数初始化
	GetPitchYawGxGyGz();
	//启动后根据磁力计的数据初始化四元数
...
//	printf("ok\n");
	...
	if(time_tick_1ms%4 == 0)         //motor control frequency 4ms
	{
		...
		//printf("%d\n",Get_Time_Micros()-time_old);
	//	time_old=Get_Time_Micros();
		//监控任务
	...
	//	printf("ax:%d,ay:%d,az:%d,gx:%d,gz:%d,gz:%d\n",MPU6050_Raw_Data.Accel_X,MPU6050_Raw_Data.Accel_Y,MPU6050_Raw_Data.Accel_Z,MPU6050_Raw_Data.Gyro_X,MPU6050_Raw_Data.Gyro_Y,MPU6050_Raw_Data.Gyro_Z);
	//	printf("ch0:%d ch1:%d ch2 %d ch3 %d\n",RC_CtrlData.rc.ch0,RC_CtrlData.rc.ch1,RC_CtrlData.rc.ch2,RC_CtrlData.rc.s2);
	}
...
void WorkStateFSM(void)
{
		...
		LostCounterFeed(GetLostCounter(LOST_COUNTER_INDEX_NOCALI));//LFM
	LostCounterFeed(GetLostCounter(LOST_COUNTER_INDEX_ZGYRO)); //LFM
...
void GMPitchControlLoop(void)
{
	...
	GMPPositionPID.ref = (100+GimbalRef.pitch_angle_dynamic_ref);
	GMPPositionPID.fdb = GMPitchEncoder.ecd_angle ;    //加入斜坡函数
	GMPPositionPID.Calc(&GMPPositionPID);   //得到pitch轴位置环输出控制量
//	printf("pitch:%.2f\n",GMPitchEncoder.ecd_angle);
	//pitch speed control
	GMPSpeedPID.ref = GMPPositionPID.output;
	GMPSpeedPID.fdb = -MPU6050_Real_Data.Gyro_X;
//	printf("y:%.2f\n",-MPU6050_Real_Data.Gyro_X);
...
void SetGimbalMotorOutput(void)
{
	...
	{		
	//	Set_Gimbal_Current(CAN1, (int16_t)GMPSpeedPID.output,  0);     //yaw + pitch		
	Set_Gimbal_Current(CAN1,(int16_t)GMPSpeedPID.output,  -(int16_t)GMPSpeedPID.output);     //yaw + pitch				
	//	printf("%f\n",GMPSpeedPID.output);
	}		
}
//控制任务初始化程序
