...
{  
	...
	else
	{
		...
		if(RC_CtrlData.key.v==KEY_E){					//按下e
			CMRotatePID.ref = -ModeCYaw;
		CMRotatePID.fdb = MPU6050_Real_Data.Gyro_Z/10.0f;
	//	printf("ref:%.2f,fdb:%f\n",CMRotatePID.ref,CMRotatePID.fdb);
		 CMRotatePID.Calc(&CMRotatePID);   
		 ChassisSpeedRef.rotate_ref = -CMRotatePID.output;
		}
		else if(RC_CtrlData.key.v==KEY_Q||shibieMode2||ReSetKey){				//按下q或者进入打大神符的模式
		 ChassisSpeedRef.rotate_ref = 0;
		}
		else {																					//正常模式
		 //底盘跟随编码器旋转PID计算
...
//	WorkStateSwitchProcess();
	...
	//平台稳定启动后，复位陀螺仪模块
	if(time_tick_1ms == PREPARE_TIME_TICK_MS*1/2)
	{
		GYRO_RST();
	}
	...
	switch(GetWorkState())
	{
		...
		case NORMAL_STATE:
		{
			...
			GMYPositionPID.ref = GimbalRef.yaw_angle_dynamic_ref;   //设定给定值
				if (shibieMode&&shibieMode2){
				GMYPositionPID.ref = -shibieYaw;
			...
			//		printf("yaw:%f\n",shibieYaw);
				...
				GMYPositionPID.ref = 0;
					...
					GMYPositionPID.fdb = -GMYawEncoder.ecd_angle;
					 ...
					 GMYPositionPID.fdb = ZGyroModuleAngle; 
				} 
...
void GMPitchControlLoop(void)
{
	...
	if (shibieMode&&shibieMode2){
		GMPPositionPID.ref = GMPitchEncoder.ecd_angle+(shibiePitch-GMPitchEncoder.ecd_angle)*PPRamp.Calc(&PPRamp);
	GimbalRef.pitch_angle_dynamic_ref=shibiePitch;
		if((shibiePitch-GMPitchEncoder.ecd_angle<3)&&(shibiePitch-GMPitchEncoder.ecd_angle>-3)){
