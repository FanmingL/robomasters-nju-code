...
RampGen_t PYRamp = RAMP_GEN_DAFAULT;   //YAW
RampGen_t PPRamp = RAMP_GEN_DAFAULT;   //PITCH
...
{
	return (fabs(ChassisSpeedRef.forward_back_ref)>=1 || fabs(ChassisSpeedRef.left_right_ref)>=1 || fabs(GimbalRef.yaw_speed_ref)>=1 || fabs(GimbalRef.pitch_speed_ref)>=1);
}
...
void RemoteControlProcess(Remote *rc)
{
    ...
    {
        ChassisSpeedRef.forward_back_ref = -(RC_CtrlData.rc.ch0 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT;
        ChassisSpeedRef.left_right_ref   = (rc->ch1 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT; 
    }
...
void MouseKeyControlProcess(Mouse *mouse, Key *key)
{
    ...
    if(GetWorkState()!=PREPARE_STATE)
    {
		...
		if(key->v==(KEY_CTRL|KEY_Q)){
			shibieMode2=1;
		}
		if(key->v==(KEY_CTRL|KEY_E)){
			shibieMode2=0;
			shibieMode=0;
		}
		
		if (key->v==(KEY_CTRL|KEY_A))info[14]=1;
		else if(key->v==(KEY_CTRL|KEY_S))info[14]=2;
		else if(key->v==(KEY_CTRL|KEY_D))info[14]=3;
		else if(key->v==(KEY_CTRL|KEY_F))info[14]=4;
		else if(key->v==(KEY_CTRL|KEY_Z))info[14]=5;
		else if(key->v==(KEY_CTRL|KEY_X))info[14]=6;
		else if(key->v==(KEY_CTRL|KEY_C))info[14]=7;
		else if(key->v==(KEY_CTRL|KEY_V))info[14]=8;
		else if(key->v==(KEY_CTRL|KEY_B))info[14]=9;
		else info[14]=0xff;
		
			
	}
	//step2: gimbal ref calc
    if(GetWorkState() == NORMAL_STATE||GetWorkState() == STANDBY_STATE)
    {
		VAL_LIMIT(mouse->x, -150, 150); 
		VAL_LIMIT(mouse->y, -150, 150); 
		if (RC_CtrlData.key.v!=KEY_E||(shibieMode&&shibieMode2)){
		...
		else if(RC_CtrlData.key.v==KEY_E){
				ModeCYaw = mouse->x* MOUTH_TO_YAW_CYAW;
...
void RemoteTaskInit()
{
	...
	PYRamp.SetScale(&PYRamp, PY_RAMP_TICK_COUNT);
	PPRamp.SetScale(&PPRamp, PP_RAMP_TICK_COUNT);
	frictionRamp.ResetCounter(&frictionRamp);
	LRSpeedRamp.ResetCounter(&LRSpeedRamp);
	FBSpeedRamp.ResetCounter(&FBSpeedRamp);
	PYRamp.ResetCounter(&PYRamp);
	PPRamp.ResetCounter(&PPRamp);
	//底盘云台给定值初始化
...
void printKey(void){
	u16 _temp = RC_CtrlData.key.v;
	if (_temp==KEY_W)printf("W\n");
	else if(_temp==KEY_A)printf("A\n");
	else if(_temp==KEY_S)printf("S\n");
	else if(_temp==KEY_D)printf("D\n");
	else if(_temp==KEY_F)printf("F\n");
	else if(_temp==KEY_G)printf("G\n");
	else if(_temp==KEY_Z)printf("Z\n");
	else if(_temp==KEY_X)printf("X\n");
	else if(_temp==KEY_C)printf("C\n");
	else if(_temp==KEY_V)printf("V\n");
	else if(_temp==KEY_B)printf("B\n");
	else if(_temp==KEY_Q)printf("Q\n");
	else if(_temp==KEY_E)printf("E\n");
	else if(_temp==KEY_R)printf("R\n");
